...
function init() {
  ...
  var map = new ymaps.Map("map", {
    ...
    //определяем центр карты (http://webmap-blog.ru/tools/getlonglat-ymap2.html)
    zoom: 17,
    // определяем уровеь масштабирования
    controls: ["zoomControl"],
    // выбираем какие элементы управления отоброжать
  ...
  }, //изображение хинта
  {
    ...
    //название
    iconImageHref: "app-images/marker.png",
    //источник
...
var modals = function () {
  var modal = document.querySelector(".modal"),
      ...
      modalExit = document.querySelector(".modal__exit"); // Открытие окна
  ...
  window.addEventListener("keydown", outsideKeypress); // Событие отправки данных с формы

  modalForm.addEventListener("submit", function (event) {
    event.preventDefault();

    if (modalForm.querySelector('[type="text"]').value === "" || modalForm.querySelector('[type="email"]').value === "") {
      modalForm.querySelector('[type="text"]').classList.add("error");
      modalForm.querySelector('[type="email"]').classList.add("error");
      setTimeout(function () {
        modalForm.querySelector('[type="text"]').classList.remove("error");
        modalForm.querySelector('[type="email"]').classList.remove("error");
